...
const listener = () => {
    ...
    //         const fileInput = document.querySelector(".images_upload");

    //         Array.from(fileInput.files).forEach((img) => console.log(img))
    ...
    //         console.log("info requesting... ", post_as_anonymous, context,images);
    ...
    monitor_new_feedback_form && monitor_new_feedback_form.addEventListener("submit", async (e) => {
        ...
        Array.from(fileInput.files).forEach((img, index) => {
        ...
        try {
            ...
            const data = JSON.parse(text); // Try parsing the raw response as JSON
